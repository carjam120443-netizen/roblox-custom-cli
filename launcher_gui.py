...
Launches Roblox through the installed official Roblox client using its
registered roblox-player protocol. It never reads credentials, cookies,
session tokens, or authentication databases.
"""
...
def refresh_status(labels: dict[str, tk.Label]) -> None:
    ...
    labels["website"].config(text="● Web login is not used for launching", fg=GOOD)
...
def main() -> None:
    ...
    tk.Label(body, text="Launch Roblox directly through the installed client.", font=("Segoe UI", 11), fg=MUTED, bg=BG).pack(anchor="w", pady=(5, 20))
    ...
    make_button("Launch Roblox", launch_roblox, True).pack(side="left")
    ...
    tk.Label(body, text=f"Version {VERSION}  •  Official client protocol  •  No credentials or session tokens are read", font=("Segoe UI", 9), fg=MUTED, bg=BG).pack(anchor="w", side="bottom")
    root.mainloop()
